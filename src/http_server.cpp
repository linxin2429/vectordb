...
void HttpServer::searchHandler(const httplib::Request& req,
    ...
    // 获取请求参数中的索引类型
    vectordb::IndexFactory::IndexType indexType =
        getIndexTypeFromRequest(json_request);

    // 如果索引类型为UNKNOWN，返回400错误
    if (indexType == vectordb::IndexFactory::IndexType::UNKNOWN) {
    ...
    void* index = vectordb::getGlobalIndexFactory()->getIndex(indexType);

    // 根据索引类型初始化索引对象并调用search_vectors函数
    std::pair<std::vector<long>, std::vector<float>>
        results;  // 直接声明results变量
    switch (indexType) {
        case vectordb::IndexFactory::IndexType::FLAT: {
            vectordb::FaissIndex* faissIndex =
                static_cast<vectordb::FaissIndex*>(index);
            results = faissIndex->search_vector(query, k);
            break;
        }
        // 在此处添加其他索引类型的处理逻辑
        default:
...
void HttpServer::setJsonResponse(const rapidjson::Document& json_response,
    ...
    res.set_content(buffer.GetString(),
                    RESPONSE_CONTENT_TYPE_JSON);  // 使用宏定义
}
...
vectordb::IndexFactory::IndexType HttpServer::getIndexTypeFromRequest(
    const rapidjson::Document& json_request) {
    if (json_request.HasMember(REQUEST_INDEX_TYPE)) {
        std::string index_type_str =
            json_request[REQUEST_INDEX_TYPE].GetString();
